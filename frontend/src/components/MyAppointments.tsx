...
export function MyAppointments() {
  ...
  const now = Date.now();
  const upcoming = sorted.filter((a) => new Date(a.scheduled_time).getTime() >= now);
  const past = sorted.filter((a) => new Date(a.scheduled_time).getTime() < now);
                    ...
                    <Card key={a.id} className="gap-0 shadow-sm">
                      <CardContent className="px-4 py-3 flex flex-wrap items-center justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">{when.toLocaleString()}</p>
                        </div>
                        <Badge className={`${statusClass(a.status)} text-xs py-0.5`}>
                          {a.status}
                        </Badge>
